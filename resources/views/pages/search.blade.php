...
@section('content')
    ...
    <div class="breadcrumb-default">
        <div class="container">
            <h1>{{ trans('layout.search_result') }} "{{ $model->seriesTitle }}" <span>{{ $model->countSearchProducts }}</span></h1>
            <ol class="breadcrumb">
                    ...
                    <div class="grid-item">
                        <div class="row">
                            @if($model->searchProducts->count() > 0)
                                @php($counter = 0)
                                @foreach($model->searchProducts as $product)
                            ...
                            <ul class="pagination">
                                ...
                                @if($isPrev)
                                    <li class="page-item">
                                        <a class="page-link previous"
                                           href="{{ url_search_per_page($model->series . ($model->sort == 'default' ? '' : '/' . $model->sort), $model->page - 1, $model->language) }}"
                                           aria-label="Previous">
                                ...
                                @foreach($pages as $page)
                                    ...
                                    @else
                                        ...
                                        @else
                                            <li class="page-item">
                                                <a href="{{ url_search_per_page($model->series . ($model->sort == 'default' ? '' : '/' . $model->sort), $page, $model->language) }}"
                                                   class="page-link">
                                                    {{ $page }}
                                                </a>
                                            </li>
